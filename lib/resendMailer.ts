...
export async function sendCampaignSequence(
  ...
  console.log(`📧 [Resend Mailer] Starting send for campaign ${args.campaignId}`);
  console.log(`📧 [Resend Mailer] Recipients: ${args.recipients.length}, Templates: ${args.emailSequence.length}`);

  const resendApiKey = process.env.RESEND_API_KEY?.trim();
  const fromEmail = process.env.RESEND_FROM_EMAIL?.trim() || process.env.SMTP_FROM?.trim();

  if (!resendApiKey) {
    console.error(`❌ [Resend Mailer] RESEND_API_KEY is not set`);
    return {
      skipped: true,
      reason: "Resend API key is not configured. Set RESEND_API_KEY in environment variables.",
      attempted: 0,
      sent: 0,
      failed: 0,
      failures: [],
  ...
  if (!fromEmail) {
    console.error(`❌ [Resend Mailer] FROM email is not set`);
    return {
      skipped: true,
      reason: "From email is not configured. Set RESEND_FROM_EMAIL or SMTP_FROM in environment variables.",
      attempted: 0,
      sent: 0,
      failed: 0,
      failures: [],
  ...
  console.log(`📧 [Resend Mailer] Using from email: ${fromEmail}`);
  ...
  for (let i = 0; i < recipientBatch.length; i++) {
    ...
    for (let j = 0; j < emailSequence.length; j++) {
      ...
      try {
        ...
        console.error(`❌ [Resend] Failed to send to ${recipient.email}: ${errorMsg}`);
        failures.push({
